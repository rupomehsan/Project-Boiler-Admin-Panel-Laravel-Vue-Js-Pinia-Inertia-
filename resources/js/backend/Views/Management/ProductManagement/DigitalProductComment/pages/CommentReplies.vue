      ...
      <div class="col-sm-12">
        <div class="card">
            ...
            <div class="row align-items-center">
              <div class="col-12 col-md-6 mb-2 mb-md-0">
                <div class="d-flex align-items-center flex-wrap" style="gap: 10px">
                  ...
                  <span
                    v-if="commenter_name"
                    style="display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: 20px; background: #e8f4fd; border: 1px solid #bee5f5; font-size: 0.8rem; font-weight: 600; color: #0c6f9f;"
                  >
                    <i class="fa fa-user-circle" style="font-size: 0.9rem"></i>{{ commenter_name }}
                  </span>
                </div>
                <p
          ...
          <div class="card-body" style="border-bottom: 1px solid #dee2e6">
            <h6 style="font-size: 0.88rem; font-weight: 600; margin-bottom: 10px">
              <i class="fa fa-plus-circle mr-1" style="color: #6f42c1"></i>Add New Reply
            </h6>
            <div class="mb-2">
              <input
            ...
            <div class="mb-2">
              ...
              ></textarea>
              <small v-if="new_reply_error" class="text-danger" style="font-size: 0.8rem">{{ new_reply_error }}</small>
            </div>
            <button
              class="btn btn-sm"
          ...
          <div class="card-body p-0">
            <div
              ...
              style="padding: 8px 16px; background: #f8f9fa; border-bottom: 1px solid #dee2e6; font-size: 0.82rem;"
            >
              <strong>Total Replies:</strong>
              <span
                style="padding: 1px 8px; border-radius: 10px; background: #d1ecf1; color: #0c5460; font-weight: 600; margin-left: 4px;"
                >{{ total_replies }}</span
              >
            </div>
              ...
              <table class="table table-hover table-bordered mb-0 text-center">
                ...
                <tbody v-if="replies.length">
                  <tr v-for="reply in replies" :key="reply.id">
                    <td>{{ reply.id }}</td>
                    <td>{{ reply.user ? reply.user.name : reply.name || "-" }}</td>
                    ...
                    <td>
                      <span
                        ...
                        :style="reply.status === 'active' ? 'background:#d4edda;color:#155724;' : 'background:#fff3cd;color:#856404;'"
                        >{{ reply.status || "active" }}</span
                      >
                    </td>
                    <td style="font-size: 0.82rem">{{ format_date(reply.created_at) }}</td>
                    <td>
                      <button
                        class="btn btn-sm"
                        style="font-size: 0.73rem; background: #dc3545; color: #fff;"
                        @click="delete_reply(reply.id)"
                      >
                  ...
                  <tr>
                    <td colspan="6">
                      <div class="d-flex flex-column align-items-center py-5">
                        <div
                          style="width: 100px; height: 100px; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, #6f42c1, #a78bfa); border-radius: 12px; opacity: 0.55;"
                        >
                          <i class="fa fa-comments" style="font-size: 50px; color: #fff"></i>
                        </div>
                        <h5 class="text-muted mt-3">No Replies Yet</h5>
                        <p class="text-secondary">Be the first to reply using the form above.</p>
                      </div>
                    </td>
                  </tr>
                </tbody>
            ...
            >
              ...
              <div v-if="pagination.last_page > 1" class="d-flex flex-wrap" style="gap: 4px">
                <button
                  ...
                  :style="pagination.current_page === 1 ? 'background:#e9ecef;color:#aaa;cursor:not-allowed;' : 'background:#17a2b8;color:#fff;'"
                ...
                <button
                  ...
                  :key="page"
                  class="btn btn-sm"
                  :style="pagination.current_page === page ? 'background:#17a2b8;color:#fff;' : 'background:#e9ecef;color:#333;'"
                  style="min-width: 32px"
                ...
                <button
                  ...
                  :style="pagination.current_page === pagination.last_page ? 'background:#e9ecef;color:#aaa;cursor:not-allowed;' : 'background:#17a2b8;color:#fff;'"
                  style="min-width: 32px"
                  @click="go_to_page(pagination.current_page + 1)"
                >&raquo;</button>
              </div>
...
export default {
  ...
  methods: {
    async load_replies() {
      ...
      try {
        const res = await axios.get(
          `${this.api_base}/digital-product-comments/${this.comment_id}/replies`,
    ...
    async submit_new_reply() {
      ...
      if (!text) { this.new_reply_error = "Reply is required."; return; }
      if (text.length < 5) { this.new_reply_error = "Reply must be at least 5 characters."; return; }

      this.new_reply_loading = true;
      try {
        const payload = { parent_id: this.comment_id, comment: text };
        if (this.new_reply_name.trim()) payload.name = this.new_reply_name.trim();
        ...
        if (e.response?.data?.errors)
          this.new_reply_error = Object.values(e.response.data.errors).flat().join(" ");
